...
test('a fresh database gets the lazy merchant tables, then every migration', async () => {
  ...
  expect(Object.fromEntries(rows.map((r) => [r.table_schema, r.n]))).toEqual({
    ...
    ledger: 3,
    merchant: 22, // with shop_hours and shop_closures (0012), shifts, shift_breaks and staff_hours (0013), setup_marks (0014), bank_accounts (0018), bank_withdrawals (0019), statement_sends (0020), terms_codes (0021)
    payments: 14,
  });
