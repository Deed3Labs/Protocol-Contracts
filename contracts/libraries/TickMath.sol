...
/// Q64.96 numbers. Supports prices between 2**-128 and 2**128.
/// @dev Vendored from Uniswap V3 Core (contracts/libraries/TickMath.sol, GPL-2.0-or-later)
/// because this repo does not depend on the uniswap v3-core package. Only getSqrtRatioAtTick is
/// needed here:
/// the AssuranceOracle reads a time-weighted average tick from a pool's observation buffer and
/// converts it back to a sqrt price to reuse the existing price math.
///
/// The only change from upstream is the `unchecked` block, which preserves the wrapping
/// arithmetic the original relied on under Solidity <0.8.
library TickMath {
