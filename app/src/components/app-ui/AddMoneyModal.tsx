...
import { ArrowLeft, Check, ChevronDown, CreditCard, Landmark, Loader2, Smartphone, ShieldCheck, Sparkles, type LucideIcon } from 'lucide-react';
...
// Linked wallets — verified by connecting + signing. The AppKit-connected wallet is the
// primary; funds always go to one of these (never an arbitrary address). USDC lands on Base.
...
export default function AddMoneyModal({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  const wallet = LINKED_WALLETS.find((w) => w.id === walletId) ?? LINKED_WALLETS[0];
