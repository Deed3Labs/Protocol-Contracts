...
export type EncryptedMemberPrivatePayload = {
  ciphertext: Buffer;
  iv: Buffer;
  authTag: Buffer;
...
function parseKeyringFromEnv(): Keyring {
  const rawKeyring = process.env.MEMBER_PRIVATE_DATA_KEYRING_JSON || '';
  const activeVersion =
    (process.env.MEMBER_PRIVATE_DATA_ACTIVE_KEY_VERSION || DEFAULT_ACTIVE_KEY_VERSION).trim()
    || DEFAULT_ACTIVE_KEY_VERSION;
  ...
  if (rawKeyring) {
    ...
    try {
      ...
      throw new Error('MEMBER_PRIVATE_DATA_KEYRING_JSON must be valid JSON');
    }
  ...
  if (keys.size === 0) {
    const fallbackMasterKey = process.env.MEMBER_PRIVATE_DATA_MASTER_KEY || '';
    if (!fallbackMasterKey) {
      throw new Error('Member private data encryption key is not configured');
    }
...
function buildAad(context: string): Buffer {
  return Buffer.from(`member-private:${context}`, 'utf8');
}

export function isMemberPrivateDataEncryptionConfigured(): boolean {
  try {
    getKeyring();
    return true;
  } catch {
...
export function encryptMemberPrivateData(
  plaintext: string,
  context: string
): EncryptedMemberPrivatePayload {
  ...
  wrapCipher.setAAD(Buffer.from(`member-datakey:${activeKeyVersion}`, 'utf8'));
...
export function decryptMemberPrivateData(
  payload: EncryptedMemberPrivatePayload,
  context: string
): string {
  const keyring = getKeyring();
  ...
  const unwrapDecipher = crypto.createDecipheriv(
    'aes-256-gcm',
    wrappingKey,
    payload.wrappedKeyIv
  );
  unwrapDecipher.setAAD(Buffer.from(`member-datakey:${payload.keyVersion}`, 'utf8'));
